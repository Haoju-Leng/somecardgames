...
const Move = ({ move, index, previousMove, gameId }) => {
  const duration = (Date.parse(move.start) - previousMove) / 1000;
  ...
  return (
    <tr>
      ...
      <th>
        ...
        <th><Link to={`/results/${gameId}/moves/${move._id}`}>{move.move}</Link></th>
    </tr>
  );
};
...
const MovesList = ({ moves, start,gameId }) => {
  let previousMove = start;

  let moveElements = moves.map((move, index) => {
      if(index !== 0){
          previousMove = Date.parse(moves[index - 1].start);
      }
      return <Move key={index} move={move} index={index} previousMove={previousMove} gameId={gameId}/>;
  });
  return (
    <MovesListTable>
      <thead>
  ...
  return (
    <ResultsBase>
      ...
      {typeof game.moves === "number" ? (
        ...
        <MovesList moves={game.moves} start={game.start} gameId={id}/>
      )}
    </ResultsBase>
  );
};
